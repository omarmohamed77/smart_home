#include"PWM.h"
 ...
 SYSCTL_RCGC2_R     |= (1<<5);      //activate port E clock
 ...
 PWM0_2_CMPA_R   =  (2500-1);        //set couter value at which CmpA triggers (any value now)
...
int duty_c=((2500*potiometer_reading)/64);
 ...
 PWM0_2_GENA_R  |=  (0xC2);         //drive pwm low on CmpA and high on zero(low logic)
 PWM0_2_LOAD_R   =  (5000-1) ;       //5000 (16khz pwm)
 PWM0_2_CMPA_R   =  (duty_c);        //set couter value at which CmpA triggers (any value now)
 PWM0_2_CTL_R   |=  (0x01);         //enable pwm block
 PWM0_ENABLE_R  |=  (PWM_ENABLE_PWM0EN);//Enable pwm output to M0PWM0
}
