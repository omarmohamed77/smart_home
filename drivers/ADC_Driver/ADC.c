...
void ADC0_Init(void)
{
	SYSTCL_RCGC0_R |= 0x0000001;//Enable clock for ADC0
	...
	GPI0_PORTE_DEN_R &=~(0x04);//disable digital on E2
	GPIO_PORTE_AMSL_R |=0x04;
...
uint16 ADC0_Reading(uint16 data)
{
	...
	while((ADC0_RIS_R & 0x8)!=0x8);//if INT3 is not set"checking if conversion is complete"
	data = ADC0_SSFIFO3_R & 0xFFF;//Storing the Sampled reading
	ADC0_ISC_R = 0x8;//clearing RIS of INT3
	return data;
}
